...
This example demonstrates how to receive CAN messages over the CAN bus using XMC&trade; MCU. This example can be used together with the [CAN Transmitter](https://github.com/cypresssemiconductorco/mtb-example-xmc-can-transmitter) example running on another board.
...
**Note:** The CAN bus lines (CANH and CANL signals) of the onboard transceiver in the KIT_XMC47_RELAX_V1 kit are not terminated by a 120-ohm resistor and needs to be terminated externally. Add a 120-ohm termination resistor between the CANH and CANL pins (X2.33 and X2.35 respectively) for the KIT_XMC47_RELAX_V1 kit.

Two XMC boards are required to test this code example: one for the application XMC MCU: CAN Transmitter and another for the application [XMC MCU: CAN Transmitter](https://github.com/cypresssemiconductorco/mtb-example-xmc-can-transmitter). See [Supported Kits](#supported-kits) for the list of kits supported by this code example.

Connect the CANH and CANL of one board to the CANH and CANL of the other board respectively to establish CAN communication between the transmitter and the receiver node. Also, connect the Ground pin of both the boards together.

| Development Board | Signal Name | Pin Header on the Board |
| -- | -- | -- |
...
3. Connect the second board running the CAN Transmitter example as described in [Hardware Setup](#hardware-setup) section.

4. Confirm that USER LED1 of the Receiver board toggles, indicating that the CAN messages are received successfully.

5. Press the User button on Transmitter board and confirm that USER LED2 on the Receiver board is updated accordingly.

6. To view the received CAN message in a serial terminal, open a terminal program and select the **JLink CDC COM port**. Set the serial port parameters to 8N1 and 115200 baud. Confirm that the received CAN message is displayed on the serial terminal as shown below:
